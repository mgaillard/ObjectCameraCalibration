...
class Images(object):
    ...
    @staticmethod
    def list_texts_directory(dir_path):
        """
        ...
        for filename in listdir(dir_path):
            path = join(dir_path, filename)
            if isfile(path) and path.lower().endswith(('.txt')):
                files.append(filename)
        files.sort()
        return files
    ...
    def read(cls, file_path, target_size=None):
        ...
        Read a single image
        """
        return cls._load_image(file_path, target_size)
    ...
    def read_list(cls, directory_path, image_files, target_size=None):
        ...
        for img in image_files:
            img = image.load_img(join(directory_path, img), target_size)
            img_array = image.img_to_array(img)
            img_array = np.expand_dims(img_array, axis=0)
    ...
    @classmethod
    def read_list_txt(cls, directory_path, txt_files):
        """
        Load all text files given in a list in a directory
        """
        data = []
